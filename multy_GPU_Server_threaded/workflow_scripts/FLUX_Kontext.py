...
class FLUX_Kontext:
    ...
    def _setup_device(self):
        ...
        if self.gpu_id is not None and self.gpu_id >= 0:
            ...
            try:
                ...
                print(f"FLUX_Kontext workflow bound to GPU {self.gpu_id}")
                return device
            except Exception as e:
                print(f"Failed to set GPU {self.gpu_id}, falling back to CPU: {e}")
                return "cpu"
        else:
            print("FLUX_Kontext workflow running on CPU")
            return "cpu"
    ...
    def start_load_once(self):
        ...
        try:
            ...
            if self.device.startswith("cuda") and torch.cuda.is_available():
                ...
                print(f"FLUX_Kontext: Loading models on {self.device}")
    ...
    def load_once(self):
        ...
        with torch.inference_mode():
            print(f"Loading FLUX_Kontext nodes on {self.device}...")
    ...
    def generate(self, workflow_name: str, image_bytes: bytes, animal_type: str, first_name: str, last_name: str, animal_name: str) -> io.BytesIO:
        ...
        try:
                ...
                try:
                    if self.device.startswith("cuda") and torch.cuda.is_available():
                        ...
                        if device_id < torch.cuda.device_count():
                            ...
                            print(f"FLUX_Kontext: Set device to {self.device}")
                        else:
                        ...
                        print(f"FLUX_Kontext: Using CPU mode")
            ...
            print(f"Error in FLUX_Kontext generate method: {e}")
            import traceback
            traceback.print_exc()
            raise